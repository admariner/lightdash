import {
    EMPTY_CONFIG,
    LOCAL_PROJECT,
    LOCAL_PROJECT_MISSING_PROFILES_DIR, LOCAL_PROJECT_MISSING_PROJECT_DIR,
    LOCAL_PROJECT_NO_PORT,
    LOCAL_PROJECT_NON_INT_PORT,
    LOCAL_PROJECT_PORT_AS_STRING,
    NO_PROJECTS,
    REMOTE_PROJECT, REMOTE_PROJECT_INVALID_HOST,
    UNDEFINED_CONFIG,
...
test('Should throw ParseError for local project with string port', () => {
    expect(() => parseConfig(wrapProject(LOCAL_PROJECT_PORT_AS_STRING))).toThrowError(ParseError);
})

test('Should throw ParseError for invalid port', () => {
    expect(() => parseConfig(wrapProject(LOCAL_PROJECT_NON_INT_PORT))).toThrowError(ParseError);
});

test('Should throw ParseError for missing profiles dir', () => {
    expect(() => parseConfig(wrapProject(LOCAL_PROJECT_MISSING_PROFILES_DIR))).toThrowError(ParseError);
});

test('Should throw ParseError for missing project dir', () => {
    expect(() => parseConfig(wrapProject(LOCAL_PROJECT_MISSING_PROJECT_DIR))).toThrowError(ParseError);
});
...
test('Should throw ParseError for invalid hostname', () => {
    expect(() => parseConfig(wrapProject(REMOTE_PROJECT_INVALID_HOST))).toThrowError(ParseError);
});
